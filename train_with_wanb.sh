...
for i in ${!envs[@]}; do
    for j in ${!methods[@]}; do
        echo CUDA_VISIBLE_DEVICES=$((i + 0)) python -W ignore general_train.py --env ${envs[$i]} --method ${methods[$j]} --record_video True --save_id $DATE --wandb_activate True --wandb_entity quantumiracle output log to: log/$DATE/${envs[$i]}_${methods[$j]}.log &
        CUDA_VISIBLE_DEVICES=$((i + 0)) nohup python -W ignore general_train.py --env ${envs[$i]} --method ${methods[$j]} --record_video True --save_id $DATE --wandb_activate True --wandb_entity quantumiracle  >> log/$DATE/${envs[$i]}_${methods[$j]}.log &
