...
from mars.equilibrium_solver import NashEquilibriumECOSSolver
...
class NashDQNSpeed(DQN):
    ...
    def choose_action(self, state, Greedy=False, epsilon=None):
        ...
        if random.random() > epsilon:  # NoisyNet does not use e-greedy
            ...
            if self.update_cnt < self.warm_up:
                q_tables = q_values.reshape(-1, self.action_dims,  self.action_dims)
                actions = []
                for qt in q_tables:
                    row_q = np.average(qt, axis=-1)
                    col_q = np.average(qt.T, axis=-1)
                    actions.append([np.argmax(row_q), np.argmin(col_q)])
            else:
                # if self.args.cce:
